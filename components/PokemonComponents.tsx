...
export function PokemonList({ data, query }: PokemonListProps) {
      ...
      <Card className="w-full">
        ...
        <CardContent>
          <p className="text-muted-foreground">
            No Pokemon data available in the response.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Star className="text-chart-5 h-5 w-5" />
      ...
      <CardContent>
        <div className="grid gap-4">
          {pokemonList
            .slice(0, 20)
            .map((pokemon: Record<string, unknown>, index: number) => (
              <div
                key={index}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="bg-primary/10 flex h-12 w-12 items-center justify-center rounded-full">
                    {pokemon.image ? (
                      <Image
                        ...
                        alt={(pokemon.name as string) || "Pokemon"}
                        width={40}
                        height={40}
                  ...
                  <div>
                    <h3 className="text-lg font-semibold">
                      {capitalize(
                        safeGet(pokemon, "name", `Pokemon ${index + 1}`)
                      )}
                    </h3>
                ...
                <div className="flex gap-2">
                  {(safeGet(pokemon, "types", []) as unknown[]).map(
                    (type: unknown, typeIndex: number) => {
                      ...
                      const typeName =
                        typeof typeObj === "string"
                          ? typeObj
                          : String(
                              (typeRecord?.type as Record<string, unknown>)
                                ?.name ||
                                typeRecord?.name ||
                                typeObj
                            );
                      return (
                        <Badge
                          key={`pokemon-type-${typeName}-${typeIndex}`}
                          variant="secondary"
...
export function PokemonDetails({ data, query }: PokemonDetailsProps) {
  ...
  if (!pokemon) {
    return (
      <Card className="w-full">
        ...
        <CardContent>
          <p className="text-muted-foreground">
            No Pokemon details available in the response.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const image =
    safeGet(pokemon, "sprites.front_default", null) ||
    safeGet(pokemon, "image", null);

  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Zap className="text-chart-5 h-5 w-5" />
          {name}
        </CardTitle>
        <CardDescription>Query: {query}</CardDescription>
      </CardHeader>
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(abilities as Record<string, unknown>[]).map(
                  (ability: Record<string, unknown>, index: number) => {
                    const abilityName = String(
                      (ability.ability as Record<string, unknown>)?.name ||
                        ability.name ||
                        ability
                    );
                    const uniqueKey = `ability-${abilityName}-${index}`;
                    return (
                      <div key={uniqueKey} className="bg-muted/30 rounded p-2">
          ...
          <>
            ...
            <div>
              ...
              <div className="grid gap-2">
                {(stats as Record<string, unknown>[]).map(
                  (stat: Record<string, unknown>, index: number) => {
                    const statName = String(
                      (stat.stat as Record<string, unknown>)?.name ||
                        stat.name ||
                        `Stat ${index + 1}`
                    );
...
export function PokemonGeneric({
  data,
  query,
}: {
  data: Record<string, unknown>;
