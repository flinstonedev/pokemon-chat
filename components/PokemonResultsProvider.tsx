...
const PokemonResultsContext = createContext<PokemonResultsContextType | undefined>(undefined);

export function PokemonResultsProvider({ children }: { children: React.ReactNode }) {
    const [results, setResults] = useState<PokemonResult[]>([]);

    const addResult = useCallback((result: Omit<PokemonResult, 'id' | 'timestamp'>) => {
    ...
    return (
        <PokemonResultsContext.Provider value={{ results, addResult, clearResults, latestResult }}>
            {children}
...
export function usePokemonResults() {
    ...
    if (context === undefined) {
        throw new Error('usePokemonResults must be used within a PokemonResultsProvider');
    }
    return context;
} 